"""
表格操作服务模块
支持企业微信在线表格的读写操作，包含 TTL 5 分钟的数据缓存
"""
from typing import List, Any, Optional, Dict, Tuple
from datetime import datetime, timedelta
from core.logger import logger
from core.exceptions import CellNotFoundError, InvalidParameterError
from validators.sheet_validator import CellData, QueryCondition, QueryOperator
...
class SheetDataCache:
    ...
    def get(self, spreadsheet_id: str, sheet_id: str, range_str: str) -> Optional[Any]:
        ...
        Returns:
            Optional[Any]: 缓存的数据，过期或不存在则返回 None
        """
...
class SheetService:
    ...
    提供企业微信在线表格的读写、查询等操作，支持 Mock 模式
    """

    _mock_data: Dict[str, List[List[Any]]] = {}

    def __init__(self, wecom_service: WeComService):
        """初始化表格服务

        Args:
            wecom_service: 企业微信服务实例，通过依赖注入传入
        """
        self.wecom_service = wecom_service
        self.use_mock = not wecom_service.is_configured
        self._cache = SheetDataCache()
        if self.use_mock:
            logger.warning("企业微信未配置，使用 Mock 模式")
            self._init_mock_data()
    
    def _init_mock_data(self):
        """初始化 Mock 数据

        创建测试用的默认表格数据，仅在 Mock 模式下使用
        """
    ...
    def _mock_write(self, spreadsheet_id: str, data: List[CellData]) -> int:
        ...
        Raises:
            InvalidParameterError: 行号或列号小于 1 时抛出
        """
    ...
    def _mock_read_all(self, spreadsheet_id: str) -> List[List[Any]]:
        ...
        Returns:
            List[List[Any]]: 二维表格数据，不存在则返回默认测试数据
        """
        if spreadsheet_id not in self._mock_data:
    ...
    def _mock_read_cell(self, spreadsheet_id: str, row: int, col: int) -> Any:
        ...
        Raises:
            CellNotFoundError: 指定的行或列不存在时抛出
        """
    ...
    def _mock_read_range(
        ...
        Returns:
            List[List[Any]]: 二维表格数据，超出范围的单元格返回 None
        """
        data = self._mock_data.get(spreadsheet_id) or self._mock_data.get("test_sheet", [])
    ...
    async def query(
        ...
        Args:
            ...
            sheet_id: 工作表 ID，可选
            conditions: 查询条件列表
            logic: 条件逻辑，"and" 或 "or"，默认 "and"

        Returns:
            List[List[Any]]: 匹配条件的表格数据（包含表头）
        """
