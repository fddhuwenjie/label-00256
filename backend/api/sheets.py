...
from services import get_sheet_service
...
async def write_data(
    ...
    Args:
        request: 写入请求对象，包含以下字段：
            spreadsheet_id: 表格ID
            sheet_id: 工作表ID（可选）
            data: 要写入的单元格数据列表
        key_info: API密钥验证信息，通过依赖注入自动获取
        sheet_service: 表格服务实例，通过依赖注入自动获取

    Returns:
        WriteResponse: 写入结果响应，包含更新的单元格数量
    """
...
async def read_data(
    ...
    Returns:
        ReadResponse: 读取结果响应，包含二维表格数据和行列统计
    """
...
async def read_range(
    ...
    Returns:
        ReadResponse: 范围读取结果响应
    """
    logger.info(
        f"读取范围: spreadsheet_id={spreadsheet_id}, range=[{start_row},{start_col}]-[{end_row},{end_col}]"
    )
